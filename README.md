...
- **ClashMi 备份导入**：直接打开或接收 ClashMi 导出的 `.backup.zip`，自动列出其中 `profiles/` 下的全部配置
- **保存并分享到 Clash**：写入成功后直接唤起 ClashMi / CFA / CMFA，无需再次选文件
- **接收分享**：接收 ClashMi、CFA 等应用分享出来的配置
- **完整 GUI 编辑**：布尔值、枚举、数字、文本、对象和列表使用对应控件，不需要编辑 YAML 文本
- **大型配置优化**：导入时后台解析一次，修改时只更新对应树路径，大型节点和规则列表默认折叠并分批显示
- **保留未知字段**：未识别的新配置键原样保留，不因固定数据类而丢失
- **规则向导**：为指定包名创建 `url-test`（最低延迟）或 `fallback`（稳定优先）组，规则安全插到 `MATCH` 前
- **分流规则管理**：搜索现有规则，按类型、匹配内容和策略组添加自定义规则；重复规则不会再次插入，而会自动定位到已有项
...
> Android 沙盒禁止普通应用直接读取其他 App 的私有数据目录。ClashMi 请使用「设置 → 备份与同步 → 导出」，然后把 `.backup.zip` 分享给本应用或从本应用选择该 ZIP；无需 root / Shizuku。
...
SnakeYAML 以有序 Map / List 解析配置，未知键和值会保留并显示在 GUI 的「其他配置」分区；保存后 YAML 的缩进 / 引号风格可能被统一，**原注释不会保留**，因此重要配置请先保留备份。
